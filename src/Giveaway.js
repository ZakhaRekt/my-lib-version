...
class Giveaway extends EventEmitter {
     ...
     */
        ...
        return new Promise(async (resolve, reject) => {
            if (this.ended) {
                return reject('Раздача с таким идом  ' + this.messageID + ' уже законченна');
            }
            if (!this.channel) {
                return reject('Не могу получить канал с id сообщения ' + this.messageID + '.');
            }
            this.ended = true;
            this.endAt = Date.now();
            await this.fetchMessage().catch(() => {});
            if (!this.message) {
                return reject('Не могу проверить сообщения с id: ' + this.messageID + '.');
            }
    ...
    reroll(options) {
        return new Promise(async (resolve, reject) => {
            if (!this.ended) {
                return reject('Раздача с таким ид  ' + this.messageID + ' не законченна.');
            }
            if (!this.channel) {
                return reject('Не могу получить канал по иду раздачи ' + this.messageID + '.');
            }
            await this.fetchMessage().catch(() => {});
            if (!this.message) {
                return reject('Не могу проверить все по иду' + this.messageID + '.');
            }
